                ...
                <div class="p-6 text-gray-900">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="bg-white border rounded-xl shadow p-5">
                            ...
                            <div class="mt-4 text-gray-600 text-sm">Applications</div>
                    ...
                    <div class="mt-6 mb-10">
                        ...
                        <canvas id="userPerMonth" width="300" height="100" data-labels='@json($userLabels)' data-data='@json($userData)'></canvas>
                    </div>
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <ul class="space-y-2 text-sm">
                                        @foreach ($projectsByCategory as $index => $projectCategory)
                                            <li>
                                                <span class="inline-block w-3 h-3 rounded-full mr-2"
                                                    style="background-color: {{ json_decode($categoryChartColors)[$index] ?? '#000' }};">
                                                </span>
                                                {{ $projectCategory->category->name ?? 'Uncategorized' }} – {{ $projectCategory->total_projects }}
                                            </li>
                                        @endforeach
                                    </ul>
                                </div>
                                <div class="w-48 h-48 relative">
                                    <canvas id="projectChart"
                                        data-labels='{{ $categoryChartLabels }}'
                                        data-data='{{ $categoryChartData }}'
                                        data-colors='{{ $categoryChartColors }}'></canvas>
                                </div>
                            </div>
                                ...
                                <div>
                                    ...
                                    <ul class="space-y-2 text-sm">
                                        <li><span class="inline-block w-3 h-3 bg-purple-500 rounded-full mr-2"></span>
                                            Seeker – {{ $totalRecruiters }}</li>
                                        <li><span class="inline-block w-3 h-3 bg-pink-500 rounded-full mr-2"></span>
                                            Recruiter – {{ $totalSeekers }}</li>
                                    </ul>
                                </div>
                                <div class="w-48 h-48 relative">
                                    <canvas id="seekerRecruiterChart" class="absolute inset-0 w-full h-full"
                                        data-total-recruiters="{{ $totalRecruiters }}"
                                        data-total-seekers="{{ $totalSeekers }}"></canvas>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <tbody class="divide-y divide-gray-200">
                                            @foreach ($projectsByCategory as $index => $projectCategory)
                                                <tr>
                                                    ...
                                                    <td class="px-4 py-2">{{ $projectCategory->category->name ?? 'Uncategorized' }}</td>
                                                    <td class="px-4 py-2">{{ $projectCategory->total_projects }}</td>
                                                </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
                                    ...
                                    <table class="min-w-full divide-y divide-gray-200 text-sm">
                                        <thead class="bg-gray-100">
                                            <tr>
                                                ...
                                                <th class="px-4 py-2 text-left font-medium text-gray-700">Application
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-200">
                                            @foreach ($projectParticipants as $index => $project)
                                                <tr>
                                                    <td class="px-4 py-2">{{ $index + 1 }}</td>
                                                    <td class="px-4 py-2">{{ $project->title }}</td>
                                                    <td class="px-4 py-2">{{ $project->category->name ?? 'Uncategorized' }}</td>
                                                    <td class="px-4 py-2">
                                                        <span class="font-medium text-blue-600">{{ $project->applications_count }}</span>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
...
<script>
    async function exportPDF() {
        ...
        logoImg.onload = function() {
            ...
            const logoHeight = pageHeight * 0.5;
