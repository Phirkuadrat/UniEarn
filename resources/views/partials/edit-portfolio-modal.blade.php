<div id="editPortfolioModal"
    ...
    <div class="bg-white rounded-lg shadow-xl p-4 sm:p-6 w-full max-w-sm sm:max-w-md md:max-w-lg mx-auto relative
        ...
        <button type="button"
            ...
            onclick="hideEditPortfolioModal()">
            <svg class="h-5 w-5 sm:h-6 sm:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
            </svg>
        </button>

        <h3 class="text-xl sm:text-2xl font-bold text-gray-800 mb-4 sm:mb-6 text-center flex-shrink-0">Edit Portfolio
        </h3>

        <div class="flex-grow overflow-y-auto pr-2 custom-scrollbar">
            <form id="editPortfolioForm" action="" method="POST" class="space-y-3 sm:space-y-4"
                enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <div>
                    <label for="edit_title" class="block text-gray-700 text-xs sm:text-sm font-semibold mb-1">Project
                        Title</label>
                    <input type="text" id="edit_title" name="title" placeholder="e.g., E-commerce Website Redesign"
                ...
                <div>
                    <label for="edit_link" class="block text-gray-700 text-xs sm:text-sm font-semibold mb-1">Project
                        Link
                        (Optional)</label>
                    <input type="url" id="edit_link" name="link" placeholder="e.g., https://yourproject.com"
                ...
                <div id="deleted-image-ids-container">
                </div>

                <div>
                    <label for="edit_images" class="block text-gray-700 text-xs sm:text-sm font-semibold mb-1">Project
                        Images (Max 5)</label>
                    ...
                    <div id="edit-image-preview" class="mt-2 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    </div>
                    @error('images')
                        <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                    @enderror
...
<script>
    ...
    let editForm = document.getElementById('editPortfolioForm');

    function showEditPortfolioModal(portfolioId) {
        currentPortfolioId = portfolioId;
        editForm.action = `/portfolio/${portfolioId}`;
    ...
    function hideEditPortfolioModal() {
        editModalContent.classList.remove('scale-100', 'opacity-100');
        editModalContent.classList.add('scale-95', 'opacity-0');
        setTimeout(() => {
            editPortfolioModal.classList.add('hidden');
            resetEditModal(); // Reset form dan preview
        }, 300);
    }
    ...
    function resetEditModal() {
        ...
        document.getElementById('edit_sub_category_id').innerHTML = '<option value="">Select Sub-Category</option>';
        document.querySelectorAll('#editPortfolioModal .border-red-500').forEach(el => el.classList.remove(
            'border-red-500'));
        document.querySelectorAll('#editPortfolioModal p.text-red-500.text-xs.italic').forEach(el => el.remove());
        const imageInput = document.getElementById('edit_images');
        if (imageInput) {
            imageInput.value = '';
        }
        document.getElementById('deleted-image-ids-container').innerHTML = '';
    }

    function fetchPortfolioData(portfolioId) {
        fetch(`/portfolio/${portfolioId}/edit-data`)
            .then(response => {
                if (!response.ok) {
            ...
            .catch(error => {
                ...
                alert('Failed to load portfolio data for editing.');
            });
    }
    ...
    function populateEditForm(data) {
        ...
        categorySelect.innerHTML = `<option value="${data.category.id}" selected>${data.category.name}</option>`;
        fetchCategoriesForEdit(data.category.id);
        if (data.sub_category) {
            subCategorySelect.innerHTML =
                `<option value="${data.sub_category.id}" selected>${data.sub_category.name}</option>`;
            fetchSubCategoriesForEdit(data.category.id, data.sub_category.id);
        } else {
            fetchSubCategoriesForEdit(data.category.id);
        }
        ...
        data.images.forEach(image => {
            ...
            imgDiv.innerHTML = `
                <img src="${image.image_path}" class="w-full h-20 sm:h-24 object-cover rounded-md border border-gray-200">
                <button type="button" class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity" onclick="removeExistingImage(this, ${image.id})">
    ...
    function fetchSubCategoriesForEdit(categoryId, selectedSubCategoryId = null) {
        const subCategorySelect = document.getElementById('edit_sub_category_id');
    ...
    document.getElementById('edit_category_id').addEventListener('change', function() {
        fetchSubCategoriesForEdit(this.value);
    });
            ...
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = function(e) {
                    ...
                    imgDiv.innerHTML = `
                        ...
                        <button type="button" class="absolute top-1 right-1 bg-red-500 text-white rounded-full w-5 h-5 sm:w-6 sm:h-6 flex items-center justify-center text-xs opacity-0 group-hover:opacity-100 transition-opacity" onclick="removeNewImage(this, '${file.name}')">
    ...
    function removeExistingImage(button, imageId) {
        ...
        const deletedImageIdsContainer = document.getElementById('deleted-image-ids-container');
        const hiddenInput = document.createElement('input');
        hiddenInput.type = 'hidden';
        hiddenInput.name = 'deleted_image_ids[]';
        hiddenInput.value = imageId;
        deletedImageIdsContainer.appendChild(hiddenInput);
