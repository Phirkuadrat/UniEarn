...
class ProjectController extends Controller
{
    ...
    public function getData()
    {
        ...
        return DataTables::of($portofolios)
            ...
            ->addColumn('action', function ($row) {
                $csrf = csrf_token();
                $deleteUrl = route('project.delete', $row->id);
                return '
                <div class="flex items-center gap-2">
                    <button 
                        onclick="showProjectDetailOverlay(' . $row->id . ')" 
    ...
    public function deleteProjectAdmin($id)
    {
        $project = Project::findOrFail($id);

        $project->delete();

        return redirect()->route('project.manage')->with('success', 'Project deleted successfully!');
    }
